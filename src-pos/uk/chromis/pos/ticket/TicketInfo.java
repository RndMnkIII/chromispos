...
public final class TicketInfo implements SerializableRead, Externalizable {
    ...
    public double getArticlesCount() {
        ...
        for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
            ...
            //System.out.println("Descuento id:"+oLine.getPromotionId());
            //System.out.println("Descontado? :"+oLine.isPromotionAdded());
        }
        return dArticles;
    }
    
    public double getArticlesCount2() {
        double dArticles = 0.0;
        TicketLineInfo oLine;
        ...
        for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
            oLine = i.next();
            //System.out.println("Descuento id:"+oLine.getPromotionId());
            if(!oLine.isPromotionAdded()) dArticles += oLine.getMultiply();
        }
        return dArticles;
    }
    ...
    public double getSubTotal() {
        //double sum = 0.0;
        BigDecimal sumBD=new BigDecimal("0.0").setScale(8,RoundingMode.HALF_UP);
        for (TicketLineInfo line : m_aLines) {
            BigDecimal subvalueBD=new BigDecimal(String.valueOf(line.getSubValue())).setScale(8,RoundingMode.HALF_UP);
            //sum += line.getSubValue();
            sumBD = sumBD.add(subvalueBD);
        }
        //return sum;
        return sumBD.setScale(8,RoundingMode.HALF_UP).doubleValue();
    }
    ...
    public double getTax() {
        ...
        if (hasTaxesCalculated()) {
            for (TicketTaxInfo tax : taxes) {
                BigDecimal taxBD=new BigDecimal(String.valueOf(tax.getTax())).setScale(8,RoundingMode.HALF_UP);
                //sum += tax.getTax(); // Taxes are already rounded...
                sumBD=sumBD.add(taxBD);
            }
        } else {
            for (TicketLineInfo line : m_aLines) {
                BigDecimal taxBD=new BigDecimal(String.valueOf(line.getTax())).setScale(8,RoundingMode.HALF_UP);
                //sum += line.getTax();
                sumBD=sumBD.add(taxBD);
            }
        }
    ...
    public double getTotal() {
        BigDecimal subtotalBD=new BigDecimal(String.valueOf(getSubTotal())).setScale(8,RoundingMode.HALF_UP);
        BigDecimal taxBD=new BigDecimal(String.valueOf(getTax())).setScale(8,RoundingMode.HALF_UP);
        BigDecimal resultBD=subtotalBD.add(taxBD);
        //return getSubTotal() + getTax();
        return resultBD.setScale(8,RoundingMode.HALF_UP).doubleValue();
    }
    ...
    public String printDate3() {
        return Formats.SIMPLEDATE.formatValue(m_dDate);
    }
    
    public String printHour() {
        return Formats.HOURMIN.formatValue(m_dDate);
    }
    
    public String printDateHour() {
        return Formats.SIMPLEDATE.formatValue(m_dDate) + " " + Formats.HOURMIN.formatValue(m_dDate);
    }
    ...
    public String printArticlesCount2() {
        return Formats.DOUBLE.formatValue(getArticlesCount2());
    }
