...
// check if the old table and the new table are the same                      
    ...
    private void paintTicketLine(int index, TicketLineInfo oLine) {
        if (executeEventAndRefresh("ticket.setline", new ScriptArg("index", index), new ScriptArg("line", oLine)) == null) {
            ...
            String vl_subtotal = m_oTicket.printSubTotal();
            String vl_tax = m_oTicket.printTax();
            String vl_total = m_oTicket.printTotal();
    ...
    private void addTicketLine(ProductInfoExt oProduct, double dMul, double dPrice) {
        if (oProduct.isVprice()) {
            ...
            if (m_jaddtax.isSelected()) {
                ...
                System.out.println("m_jaddtax is selected No BigDecimal computed value!");
            }
...
// includes modified consolidate receipt code for screen and refreshes the screen afer updating     
            ...
            String vl_total = m_oTicket.printTotal();
            String vl_items = m_oTicket.printArticlesCount2();
            //System.out.println("Count:"+m_oTicket.printArticlesCount());
            //System.out.println("count2:"+vl_items);
            //visorTicketLine2(m_oTicket, oLine);
            visorTicketLine3(vl_subtotal, vl_tax, vl_total, vl_items, oLine);
...
// if this is ticket does not have a pickup code assign on now            
    ...
    public void printReport(String resourcefile, TicketInfo ticket, Object ticketext) {
        //End of modifications by RndMnkIV
        try {
            JasperReport jr;
    ...
    //Start of modifications by RndMnkIV    
    private void visorTicketLine2(TicketInfo oTicket, TicketLineInfo oLine) {
        if (oLine == null) {
            m_App.getDeviceTicket().getDeviceDisplay().clearVisor();
        } else {
            try {
    ...
    //End of modifications by RndMnkIV   
        ...
        //Start of modifications by RndMnkIV    
    private void visorTicketLine3(String subtotal, String tax, String total, String itemscount, TicketLineInfo oLine) {
        if (oLine == null) {
            m_App.getDeviceTicket().getDeviceDisplay().clearVisor();
        } else {
            try {
                ScriptEngine script = ScriptFactory.getScriptEngine(ScriptFactory.VELOCITY);
                script.put("ticketline", oLine);
                script.put("subtotal", subtotal);
                script.put("tax", tax);
                script.put("total", total);
                script.put("itemscount", itemscount);
